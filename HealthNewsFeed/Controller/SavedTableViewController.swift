...
class SavedTableViewController: UITableViewController {
    ...
    func deleteAllCoreData() {
        let fetchReguest = NSFetchRequest<NSFetchRequestResult>(entityName: "Items")
        
        let entityReguest: NSBatchDeleteRequest = NSBatchDeleteRequest(fetchRequest: fetchReguest)
        
        do {
            try context?.execute(entityReguest)
            saveCoreData()
        }catch  let error{
    ...
    @IBAction func deleteAllItemsTapped(_ sender: Any) {
        ...
        let alertController = UIAlertController(title: "Delete All data", message: "Do you want to delete your saved list?", preferredStyle: .actionSheet)
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if savedItems.count == 0 {
            tableView.setEmptyView(title: "You don't have any saved contact.", message: "Plesae go to News Tab Bar to check the articles.")
        }
        else {
            tableView.restore()
        }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "savedCell", for: indexPath) as? NewsTableViewCell else { return UITableViewCell() }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        vc.urlString = savedItems[indexPath.row].url ?? "https://www.healthline.com/health-news"
